...
Implements all DID (Decentralised Identifier) methods supported by ASDK and the `UniversalResolver` that dispatches resolution across them. Given a DID string, this domain resolves it to a W3C DID document and extracts verification methods and JWKs for use by the VC and DIDComm domains.
...
- The `one-core` `HttpClient` trait (`get/post/send` pattern) is incompatible with ASDK's (`async_call` pattern); bridging requires an adapter — see `src/did/webvh/client.rs` for the reference pattern.