...
Exposes the ASDK Rust library to three foreign language targets via thin FFI/binding layers. Each wrapper translates ASDK's async Rust types to the idioms of its target ecosystem. No business logic lives here — wrappers only adapt types, handle errors, and re-export the SDK's public API surface.
