...
| Node.js | `@equs-ai/equs-credentials-sdk` | `wrappers/nodejs/package.json` (napi binary `equs-credentials-sdk`). Published to npmjs with its three platform packages by `.github/workflows/publish-npm.yml` on an `npm/vX.Y.Z` tag (prerelease suffix → `dev` dist-tag). |
| WASM | `@equs-ai/equs-credentials-sdk-wasm` | `wrappers/wasm/package.json`. Published to npmjs by `.github/workflows/publish-npm.yml`, same tag. |
