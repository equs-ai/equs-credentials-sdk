...
## Purpose
Standalone demonstration applications showing EQUS Credentials SDK usage across multiple target environments
...
| nodejs/      | Node.js (TypeScript) OID4VC demo using the NAPI-RS bindings. See [nodejs/CLAUDE.md](nodejs/CLAUDE.md). |
| wasm/        | Browser/WASM OID4VC wallet demo using the WASM bindings. See [wasm/CLAUDE.md](wasm/CLAUDE.md). |
...
| android/     | Android Kotlin demo using the UniFFI wrapper (AAR). See [android/CLAUDE.md](android/CLAUDE.md). |
| ios/         | iOS Swift demo using the UniFFI wrapper (XCFramework). See [ios/CLAUDE.md](ios/CLAUDE.md). |
...
- All demos are consumers of the SDK; they do not define new SDK types.
...
- Used by: developers exploring the SDK's capabilities and integration patterns
...
- Demo `.rs` source files are not part of the main SDK library and should not be modified as part of SDK development.
