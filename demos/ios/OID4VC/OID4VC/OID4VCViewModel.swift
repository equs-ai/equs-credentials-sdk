...
class OID4VCViewModel : ObservableObject, AuthCodeCallback {
    ...
    func setup() async {
        ...
        do {
            self.oid4vciHolder = try await OID4VCViewModel.buildOid4vciHolder(kms: kms, vault: vault)
    ...
    @MainActor
    func startAuthorizationFlow() async {
        authorizationCodeURL = .loading
        
        do {
            let tokenResponse = try await self.oid4vciHolder.authzCodeFlowWithScope(scope: Constants.scope, authorizationCodeCallback: self)
    ...
    @MainActor
    func startCredentialIssuanceFlow(token: String) async {
        credential = .loading
        ...
        do {
            ...
            case .immediate(credential: let credential, notificationId: _):
                ...
                let _ = try await self.oid4vciHolder.storeCredential(credential: credential, credentialMetadata: metadata)
            }
        } catch {
            print("error = \(error)")
    ...
    @MainActor
    func startPresentationFlow() async {
        presentation = .loading
        
        do {
            let authorizationRequest = try await self.oid4vpHolder.getAuthorizationRequest(requestUri: requestUri)
            
            let didAndKeyMetadata = await createDidAndKeyMetadata(kms: kms)
            let idTokenMetadata = IdTokenMetadata(keyMetadata: didAndKeyMetadata.keyMetadata, lifetime: 5)
            
            let _ = try await self.oid4vpHolder.presentCredentialsAuto(authRequest: authorizationRequest, authResponseMetadata: AuthorizationResponseMetadata(claimsToExclude: nil , idTokenMetadata: idTokenMetadata))
