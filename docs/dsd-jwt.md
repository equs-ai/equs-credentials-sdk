...
See [Five-party demo](../demos/oid4vc/README.md#delegated-sd-jwt-dsd-jwt-demo) for a working demo of dSD-JWT in agentic use cases.

## Overview
dSD-JWT — a delegated SD-JWT — is a general-purpose mechanism for one party to hand a bounded, verifiable authorization to another party, on top of a credential it already holds.
...
1. The Holder signs a delegate payload — a set of claims describing what is being authorized — with the key the credential is bound to.
...
Several delegation items may name the same credential in their `credential_ids`. They then describe alternatives over that credential: all of them are signed into the grant, so a single approval can pre-authorize several mutually exclusive options.
