...
## Purpose
Optional, feature-gated EQUS Credentials SDK plugins that provide alternative backend implementations
...
## Dependencies
- Depends on: `equs_sdk` (trait definitions), third-party backends (`aries_askar`)
- Used by: host applications and demos that need persistent storage beyond in-memory mocks

## Constraints
- Plugin crates are standalone Cargo workspaces; they are not compiled as part of the main SDK workspace unless explicitly added to the workspace.
