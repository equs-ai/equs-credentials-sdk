...
## Purpose
Wraps the external `didcomm` crate to provide DIDComm V2 message packing and unpacking. Adapts ASDK's `UniversalResolver` and `Kms` types to the interfaces expected by the `didcomm` crate.
...
Supertrait: ASDK's `Kms<KH>` combined with `didcomm` crate's `KeyManagementService` adapter. Required by `Agent::new`.
