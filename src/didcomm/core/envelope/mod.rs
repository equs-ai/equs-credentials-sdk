...
use didcomm::secrets::KeyManagementService;
...
use did_resolver::DidResolverWrapper;
use kms::KmsWrapper;

pub use kms::DIDCommKms;
...
pub struct EnvelopeService {
    kms: Arc<dyn KeyManagementService + Send + Sync>,
    did_resolver: DidResolverWrapper,
}

impl EnvelopeService {
    /// Creates a new [`EnvelopeService`].
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// A new instance of [`EnvelopeService`].
    #[instrument(level = Level::TRACE, skip_all)]
    pub fn new<KMS, KH>(kms: KMS, did_resolver: UniversalResolver) -> Self
    where
        KMS: DIDCommKms<KH> + 'static,
        KH: KeyHandle + 'static,
    {
        EnvelopeService {
            kms: Arc::new(KmsWrapper::new(kms, did_resolver.clone())),
    ...
    pub async fn pack_signed(
        ...
        message
            .pack_signed(sign_by, &self.did_resolver, self.kms.deref())
            .await
            .context(Snafu)
    }
    ...
    pub async fn unpack(
        ...
        Message::unpack(msg, &self.did_resolver, self.kms.deref(), options)
            .await
            .context(Snafu)
    }
}
...
mod test {
    ...
    async fn pack_encrypted_succeeded() {
        ...
        let didcomm_service = EnvelopeService::new(kms, did_resolver);
    ...
    async fn pack_encrypted_failed_with_incorrect_sender_did() {
        ...
        let didcomm_service = EnvelopeService::new(kms, UniversalResolver::default());
    ...
    async fn pack_signed_succeeded() {
        ...
        let didcomm_service = EnvelopeService::new(kms.clone(), did_resolver);
