...
mod tests {
    ...
    use crate::didcomm::protocol::aries::issuance::issuer::{CredentialInfo, Issuer};
    ...
    async fn test_issuance() {
        ...
        let (hld_did, hld_key_metadata, _) = create_did_and_key_metadata(holder_agent.kms()).await;
    ...
    async fn test_presentation() {
        ...
        let cred_test_case = CredTestCase::ldp_vc();
        let (entry, did) = cred_test_case.generate_vc(&kms, None).await;
        let presentation_definition = cred_test_case.create_presentation_definition();

        let vault = InMemVault::new();
        vault.store_entry(&entry, did).await.unwrap();
        ...
        let (verifier_subscription, verifier_observable) =
            verifier.observe_state(thread_id.to_owned()).await;
        ...
        let (prover_did, prover_key_metadata, prover_kh) =
            create_did_and_key_metadata(prover_agent.kms()).await;
        ...
        let (prover_subscription, prover_observable) = prover.observe_state().await;
    ...
    pub async fn create_did_and_key_metadata(kms: &LocalKms) -> (DID, KeyMetadata, KeyHandle) {
        let (kid, kh) = kms
