...
## Purpose
Provides concrete in-memory implementations of all cryptographic suites supported by ASDK, implementing the `Suite`, `Signer`, `Verifier`, and `Key` traits defined in `crate::crypto`.
...
- `HasAlg` — associates a curve type with its ASDK `Alg` value.
