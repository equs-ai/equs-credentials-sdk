...
- Builds instances of `agent_sdk::vc::oid4vci::{Issuer, Holder}` using `LocalKms` + `InMemVault`.

## Dependencies
- Depends on: `agent_sdk` (inmem, kms, vc::oid4vci), `utils::http::HttpClientEmulator`, `utils::fixtures`
