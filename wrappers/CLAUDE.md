...
- **HTTP abstraction**: All three wrappers expose a `ReqwestHttpClient` backed by `equs_sdk::reqwest::ReqwestClient` as a default, and accept a caller-supplied HTTP client for customization.
...
- **Prepare/sign split**: All three wrappers expose `VCCoreCredentialSigner` (a stand-alone `SignCredential` wrapper over the SDK's `CredentialSigner`). The unsigned credential travels between `prepare_credential` and `sign_credential` as the externally-tagged JSON shape of `equs_sdk::vc::core::UnsignedCredential` — `{ "SdJwt": {...} }` / `{ "Ldp": {...} }` — defined once in TypeScript at `wrappers/types/vc/unsigned-credential.ts` and shared by `nodejs`/`wasm`. The `nodejs` `VCCoreIssuer` additionally exposes `prepareCredential` so a service can prepare in Node and have a wasm/Kotlin/Swift consumer sign.
...
## Dependencies
- All wrappers depend on the `equs_sdk` crate (workspace root `src/`).
