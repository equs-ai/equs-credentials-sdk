...
- **Prepare/sign split**: All three wrappers expose `VCCoreCredentialSigner` (a stand-alone `SignCredential` wrapper over the SDK's `CredentialSigner`). The unsigned credential travels between `prepare_credential` and `sign_credential` as the externally-tagged JSON shape of `agent_sdk::vc::core::UnsignedCredential` — `{ "SdJwt": {...} }` / `{ "Ldp": {...} }` — defined once in TypeScript at `wrappers/types/vc/unsigned-credential.ts` and shared by `nodejs`/`wasm`. The `nodejs` `VCCoreIssuer` additionally exposes `prepareCredential` so a service can prepare in Node and have a wasm/Kotlin/Swift consumer sign.
