...
## Purpose
Root Rust source crate for the EQUS Credentials SDK Node.js wrapper. Uses NAPI-RS (`#[napi]` macros) to compile to a native Node.js add-on (`.node` binary). Exposes the full SDK surface — DID methods, VC issuance and presentation (OID4VCI/OID4VP), DIDComm V2 messaging, KMS, vault, HTTP client, and nonce handling — as TypeScript-typed native classes and functions.
...
## Constraints
- The `inmem` module is gated behind `#[cfg(any(test, feature = "in-memory"))]` in the SDK core but is always compiled in the Node.js wrapper.
