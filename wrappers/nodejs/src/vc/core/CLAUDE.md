...
| `issuer.rs` | `VCCoreIssuer` — raw NAPI class (`js_name = "_VcCoreIssuer"`) wrapping `Box<dyn IssuerWithPrepare>`. The public `VcCoreIssuer` TS wrapper in `types/vc/core/issuer.ts` applies `contextEnsuredKms` and unwraps `didResolver.inner`. Exposes `offerCredential`, `issueCredential`, `prepareCredential`. |
...
| `holder.rs` | `VCCoreHolder` — raw NAPI class (`js_name = "_VcCoreHolder"`) wrapping `Box<dyn Holder>`. The public `VcCoreHolder` TS wrapper in `types/vc/core/holder.ts` applies `contextEnsuredKms`, `contextEnsuredVault`, and unwraps `didResolver.inner`. Exposes `requestCredential`, `storeCredential`, `verifyCredential`, `createPresentationAuto`, `findVcsForPresentation`, `createPresentation`, `getCredentialStatus`. |
| `verifier.rs` | `VCCoreVerifier` — raw NAPI class (`js_name = "_VcCoreVerifier"`) wrapping `Box<dyn Verifier>`. The public `VcCoreVerifier` TS wrapper in `types/vc/core/verifier.ts` unwraps `didResolver.inner`. Exposes `verifyPresentation`, `obtainCredentialStatus`. |
| `status_issuer.rs` | `VCCoreStatusIssuer` — raw NAPI class (`js_name = "_VcCoreStatusIssuer"`) wrapping `Box<dyn StatusIssuer>`. The public `VcCoreStatusIssuer` TS wrapper in `types/vc/core/status-issuer.ts` applies `contextEnsuredKms`. Exposes `issueStatusList`. |
