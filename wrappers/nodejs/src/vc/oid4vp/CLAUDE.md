...
| `verifier.rs` | `InternalOID4VPVerifier` — NAPI class wrapping `Box<dyn Verifier>`. Exposes `create_authorization_request` and `verify_presentation`. Defines `JsAuthResponseOptions`, `JsPassAuthRequestObject`, `JsAuthorizationRequestMetadata`, `JsCredentialVerificationMetadata`, `JsPresentationSession`, `AuthorizationRequestWithSession`, and `JsHttpMethodForAuth`. |
