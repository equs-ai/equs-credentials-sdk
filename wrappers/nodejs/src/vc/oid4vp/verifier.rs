...
impl InternalOID4VPVerifier {
    ...
    ///     - audience - In the case of DC API response mode, audience is the bare Origin of the Verifier (e.g. `https://verifier.example.com`). In DC API response mode, the SDK binds the presentation to `origin:<Origin>`.
    ///
