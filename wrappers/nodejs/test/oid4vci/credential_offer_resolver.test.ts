...
describe("OID4VCI Credential Offer resolver: ", () => {
    ...
    test("resolve offer by reference with pre-authorized code grant", async () => {
        ...
        const resolver = new Oid4VciCredentialOfferResolver();

        const resolvedOffer = await resolver.resolve("openid-credential-offer://?credential_offer_uri=http://localhost:9000/credential_offer");
    ...
    test("resolve offer by value with pre-authorized code grant", async () => {
        ...
        const resolvedOffer = await resolver.resolve("openid-credential-offer://?credential_offer={%22credential_issuer%22:%22http://localhost:9000%22,%22credential_configuration_ids%22:[%22IDENTITY_SD_JWT%22],%22grants%22:{%22urn:ietf:params:oauth:grant-type:pre-authorized_code%22:{%22pre-authorized_code%22:%22code%22,%22tx_code%22:null,%22interval%22:null,%22authorization_server%22:%22http://localhost:9000/auth%22}}}");
