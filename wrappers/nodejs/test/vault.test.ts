import { Alg, CredentialSearchCriteria, wrapJsVault, VCFormat, Vault } from "../index";
...
describe("Vault: ", () => {
	...
	test("get Credential", async () => {
		...
		expect(entry).toEqual({ credential: data.credential, kid: data.metadata.kid });
	});
...
function mockVault(credential_data): Vault {
	return {
		...
		async findCredentials(criteria) {
			const value = await criteria.value();
			if (
				!(
					value.type === "ByTypeAndFormat" &&
					value.cred_type === credential_data.metadata.type &&
