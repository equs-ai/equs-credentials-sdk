...
| `status_formats.rs` | `SLMetadata` record + `StatusListFormat` wrapper enum bridging `agent_sdk::vc::status_formats::StatusListFormat`. |
...
- `VCCoreHolder`, `VCCoreVerifier`, `VCCoreStatusIssuer` — UniFFI objects wrapping `Box<dyn Trait>` from `agent_sdk::vc::core`.
...
## Dependencies
- Depends on: `agent_sdk::vc::core`, `agent_sdk::vc::status_formats`, `crate::common`, `crate::http`, `crate::kms`, `crate::vault`, `crate::did`
